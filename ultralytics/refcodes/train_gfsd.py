...
def train_rgb_ir(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'theta', 'distill', 'labels', 'img_size'))
        if RANK in [-1, 0]:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                pred, cls_consistency_loss = model(imgs_rgb, imgs_ir)  # forward
                loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
                #? TODO 
                # 改为命令行控制opt
                # import pdb; pdb.set_trace()
                # 增加蒸馏loss
                #TODO：11111111111111111111111111
            ...
            if RANK in [-1, 0]:
                ...
                pbar.set_description(('%10s' * 2 + '%10.4g' * 7) % (
                    f'{epoch}/{epochs - 1}', mem, *mloss, cls_consistency_loss, targets.shape[0], imgs.shape[-1]))
